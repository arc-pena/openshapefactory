...
const MODULES = [
  "fontdata.js", "geom2d.js", "expr.js", "units.js", "ocaf.js", "library.js", "sections.js", "sectionlib.js", "walls.js", "spaces.js", "joins.js", "dxf.js",
  "cadsketch.js", "bimsketch.js", "massing.js", "bim.js", "solids.js", "sun.js", "crop.js", "styles.js", "spacegraph.js", "ops.js", "scene.js", "props.js", "hlr.js", "pdf.js", "render.js", "sample.js", "sample_rmuh.js", "sample_pavilion.js", "ifcsections.js", "ifcread.js", "ifcimport.js", "cadbridge.js", "acceptance.js",
  "ui_util.js", "sectionui.js", "panel.js", "graph.js", "sketchui.js", "sgui.js", "canvas2d.js", "viewcube.js", "view3d.js", "app.js",
];
const fail = msg => { console.error("build refused: " + msg); process.exit(1); };

// The BIM sketcher runs on the parametric CAD's own sketch kernel (cad/src/sketch.js): one
// definition of a drawing - its loops, holes, fillets and solver - read by both interfaces.
// It is wrapped, not copied by hand: its helpers (add, sub, len...) would collide with ours in
// the single file, so it is closed in a function and only its exports come out, under their
// own names. Regenerated on every build, so it cannot drift from the CAD's copy.
{
  const cad = fs.readFileSync(path.join(ROOT, "cad", "src", "sketch.js"), "utf8");
  if (/^import\s/m.test(cad)) fail("cad/src/sketch.js imports something; the wrapper expects a self-contained module");
  const names = [...cad.matchAll(/^export\s+(?:const|function)\s+([A-Za-z_$][\w$]*)/gm)].map(m => m[1]);
  const body = cad.replace(/^export\s+(?=(const|let|function|class)\b)/gm, "").replace(/^(?=.)/gm, "  ");
  const out = `//! GENERATED by build.mjs from cad/src/sketch.js - do not edit; edit that file.
//! The parametric CAD's sketch kernel, closed in one scope, its exports named cad*.
const CAD_SKETCH = (() => {
${body}
return { ${names.join(", ")} };
})();
${names.map(n => `export const cad${n[0].toUpperCase()}${n.slice(1)} = CAD_SKETCH.${n};`).join("\n")}
`;
  const at = path.join(SRC, "cadsketch.js");
  if (!fs.existsSync(at) || fs.readFileSync(at, "utf8") !== out) fs.writeFileSync(at, out);
}
