//! GENERATED by build.mjs from cad/src/sketch.js - do not edit; edit that file.
//! The parametric CAD's sketch kernel, closed in one scope, its exports named cad*.
const CAD_SKETCH = (() => {
  ...
  const SKETCH_TYPES = ["point", "line", "rect", "arc", "circle", "ellipse", "oblong",
                               "spline", "bspline"];
  ...
  const EMPTY_SKETCH = { elements: [], constraints: [] };
  ...
  function sketchRelation(type, of) {
  ...
  //! many you give it.
  const SKETCH_CLICKS = { point: 1, line: 2, rect: 2, circle: 2, arc: 3, ellipse: 3,
  ...
  function sketchMoveHandle(el, key, to) {
  ...
  function sketchHandleAt(drawing, reference) {
  ...
  function sketchOutline(el, quality = 64) {
  ...
  function splinePoints(el, perSpan = 12) {
    const pts = el.pts || [];
  ...
  //! square.
  function bsplinePoints(el, perSpan = 16) {
    const given = el.ctrl || [];
  ...
  function uniformKnots(count, degree, periodic = false) {
    const knots = [];
    if (periodic) {
  ...
  const sketchGrain = drawing => Math.max(1e-4, sketchExtent(drawing) * 1e-6);
  ...
  function sketchLoops(drawing, tolerance = 0.05) {
  ...
  //! arc, not a mistake.
  function sketchOverlaps(drawing, tolerance) {
  ...
  function sketchRelationMarks(drawing) {
  ...
  function sketchDirectionAt(el, key) {
    if (!el) return null;
  ...
  function sketchTangentArc(from, tangent, to, id) {
  ...
  function sketchFillet(drawing, firstRef, secondRef, radius, id = "f1") {
  ...
  function sketchChainEnds(drawing, chain, closed = false) {
    const map = byId(drawing);
    const run = [];
  ...
  function sketchArcPoint(el, angle) { return arcEnd(el, angle); }
  ...
  function sketchLoopOutline(drawing, chain, quality = 48) {
    const map = byId(drawing);
    const out = [];
    for (const step of chain || []) {
      const el = map.get(step.id);
  ...
  function sketchNesting(drawing, loops, quality = 48) {
  ...
  function solveSketch(drawing, passes = 24, pinned = []) {
    const work = JSON.parse(JSON.stringify(drawing || EMPTY_SKETCH));
  ...
  function coincidentGroup(drawing, ref) {
    const links = new Map();
    for (const c of (drawing.constraints || [])) {
  ...
  function sketchCrossings(drawing, a, b) {
  ...
  function elementShown(drawing, el) {
    const layers = sketchLayers(drawing);
    const on = layers.find(l => l.name === layerName(el));
    return !on || on.on;
  }

  function elementLocked(drawing, el) {
    const layers = sketchLayers(drawing);
    const on = layers.find(l => l.name === layerName(el));
    return !!on && on.locked;
  }
  ...
  function sketchDistanceTo(el, p, quality = 48) {
    const line = sketchOutline(el, quality);
  ...
  function sketchInBox(drawing, box, { crossing = false, quality = 32 } = {}) {
    const out = [];
    for (const el of (drawing.elements || [])) {
  ...
  const sketchOnLayer = (drawing, name) =>
  ...
  function sketchMoveElement(el, by) {
  ...
  const isConstruction = el => !!(el && el.construction);
  ...
  function nextSketchId(drawing, prefix = "e") {
  ...
  function sketchSummary(drawing) {
...
return { SKETCH_TYPES, wholeEllipse, SKETCH_RELATIONS, EMPTY_SKETCH, sketchRound, sketchElement, sketchRelation, SKETCH_CLICKS, sketchHandles, ellipseAt, sketchMoveHandle, sketchHandleAt, sketchOutline, splinePoints, bsplinePoints, uniformKnots, reversedBspline, sketchEnds, sketchEndKeys, sketchExtent, sketchGrain, sketchLoops, sketchOverlaps, sketchRelationMarks, sketchDirectionAt, sketchTangentArc, sketchFillet, sketchChainEnds, sketchArcPoint, sketchLoopOutline, pointInPolygon, sketchNesting, solveSketch, coincidentGroup, sketchCrossings, SKETCH_LAYER, sketchLayers, currentLayer, elementShown, elementLocked, shownDrawing, sketchDistanceTo, sketchBox, sketchInBox, sketchOnLayer, sketchMoveElement, isConstruction, builtDrawing, readSketch, nextSketchId, sketchSummary };
})();
export const cadSKETCH_TYPES = CAD_SKETCH.SKETCH_TYPES;
export const cadWholeEllipse = CAD_SKETCH.wholeEllipse;
export const cadSKETCH_RELATIONS = CAD_SKETCH.SKETCH_RELATIONS;
export const cadEMPTY_SKETCH = CAD_SKETCH.EMPTY_SKETCH;
export const cadSketchRound = CAD_SKETCH.sketchRound;
export const cadSketchElement = CAD_SKETCH.sketchElement;
export const cadSketchRelation = CAD_SKETCH.sketchRelation;
export const cadSKETCH_CLICKS = CAD_SKETCH.SKETCH_CLICKS;
export const cadSketchHandles = CAD_SKETCH.sketchHandles;
export const cadEllipseAt = CAD_SKETCH.ellipseAt;
export const cadSketchMoveHandle = CAD_SKETCH.sketchMoveHandle;
export const cadSketchHandleAt = CAD_SKETCH.sketchHandleAt;
export const cadSketchOutline = CAD_SKETCH.sketchOutline;
export const cadSplinePoints = CAD_SKETCH.splinePoints;
export const cadBsplinePoints = CAD_SKETCH.bsplinePoints;
export const cadUniformKnots = CAD_SKETCH.uniformKnots;
export const cadReversedBspline = CAD_SKETCH.reversedBspline;
export const cadSketchEnds = CAD_SKETCH.sketchEnds;
export const cadSketchEndKeys = CAD_SKETCH.sketchEndKeys;
export const cadSketchExtent = CAD_SKETCH.sketchExtent;
export const cadSketchGrain = CAD_SKETCH.sketchGrain;
export const cadSketchLoops = CAD_SKETCH.sketchLoops;
export const cadSketchOverlaps = CAD_SKETCH.sketchOverlaps;
export const cadSketchRelationMarks = CAD_SKETCH.sketchRelationMarks;
export const cadSketchDirectionAt = CAD_SKETCH.sketchDirectionAt;
export const cadSketchTangentArc = CAD_SKETCH.sketchTangentArc;
export const cadSketchFillet = CAD_SKETCH.sketchFillet;
export const cadSketchChainEnds = CAD_SKETCH.sketchChainEnds;
export const cadSketchArcPoint = CAD_SKETCH.sketchArcPoint;
export const cadSketchLoopOutline = CAD_SKETCH.sketchLoopOutline;
export const cadPointInPolygon = CAD_SKETCH.pointInPolygon;
export const cadSketchNesting = CAD_SKETCH.sketchNesting;
export const cadSolveSketch = CAD_SKETCH.solveSketch;
export const cadCoincidentGroup = CAD_SKETCH.coincidentGroup;
export const cadSketchCrossings = CAD_SKETCH.sketchCrossings;
export const cadSKETCH_LAYER = CAD_SKETCH.SKETCH_LAYER;
export const cadSketchLayers = CAD_SKETCH.sketchLayers;
export const cadCurrentLayer = CAD_SKETCH.currentLayer;
export const cadElementShown = CAD_SKETCH.elementShown;
export const cadElementLocked = CAD_SKETCH.elementLocked;
export const cadShownDrawing = CAD_SKETCH.shownDrawing;
export const cadSketchDistanceTo = CAD_SKETCH.sketchDistanceTo;
export const cadSketchBox = CAD_SKETCH.sketchBox;
export const cadSketchInBox = CAD_SKETCH.sketchInBox;
export const cadSketchOnLayer = CAD_SKETCH.sketchOnLayer;
export const cadSketchMoveElement = CAD_SKETCH.sketchMoveElement;
export const cadIsConstruction = CAD_SKETCH.isConstruction;
export const cadBuiltDrawing = CAD_SKETCH.builtDrawing;
export const cadReadSketch = CAD_SKETCH.readSketch;
export const cadNextSketchId = CAD_SKETCH.nextSketchId;
export const cadSketchSummary = CAD_SKETCH.sketchSummary;
